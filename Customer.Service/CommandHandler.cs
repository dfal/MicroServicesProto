...
namespace Customer.Service
{
	class CommandHandler
	{
		...
		{
			var customer = new Customer(message.Name, message.VatNumber, message.Email);
		...
		void OnUpdateCustomer(UpdateCustomerMessage message)
		{
			...
			customer.Rename(message.Name);
			customer.ChangeEmail(message.Email);
			customer.ChangeVatNumber(message.VatNumber);
